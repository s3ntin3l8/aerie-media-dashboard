...
function Login({ onSignIn }) {
  const [phase, setPhase] = useState('idle'); // idle | redirecting
  const go = () => { setPhase('redirecting'); setTimeout(onSignIn, 1400); };

  return (
    <div style={{ height: '100%', display: 'flex', background: 'var(--background)', overflow: 'hidden' }}>
      {/* Left: brand panel */}
      <div style={{ flex: '0 0 46%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '48px 56px', background: 'var(--surface-container-lowest)', borderRight: '1px solid var(--outline-variant)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2 }}>
          <Eyebrow color="var(--primary)" style={{ marginBottom: 16 }}>Private Media Command Center</Eyebrow>
          <h1 style={{ fontFamily: 'var(--font-headline)', fontWeight: 700, fontSize: 34, lineHeight: 1.12, letterSpacing: '-0.02em', color: 'var(--on-surface)', maxWidth: 380 }}>Every service, one vantage point.</h1>
          <p style={{ fontSize: 14, lineHeight: 1.6, color: 'var(--on-surface-variant)', marginTop: 16, maxWidth: 360 }}>Streaming, requests, automation and monitoring — unified behind a single secure door. All access flows through your own identity provider.</p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 24 }}>
            {['Plex','Jellyfin','Overseerr','Sonarr','Radarr','Tautulli','Gatus'].map(n => (
              <span key={n} style={{ fontFamily: 'var(--font-mono)', fontSize: 11, padding: '4px 10px', borderRadius: 9999, border: '1px solid var(--outline-variant)', color: 'var(--on-surface-variant)', background: 'var(--surface-container-low)' }}>{n}</span>
            ))}
          </div>
        </div>
        ...
        <div style={{ width: '100%', maxWidth: 380 }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 28 }}>
            <div style={{ position: 'relative', width: 64, height: 64, marginBottom: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 18, background: 'color-mix(in srgb, var(--primary) 12%, transparent)', border: '1px solid color-mix(in srgb, var(--primary) 25%, transparent)' }}>
              <Icon name={phase === 'redirecting' ? 'sync' : 'shield_person'} size={30} color="var(--primary)" style={phase === 'redirecting' ? { animation: 'aerieSpin 1s linear infinite' } : null} />
            </div>
          ...
          <div style={{ padding: 14, borderRadius: 12, border: '1px solid var(--outline-variant)', background: 'var(--surface-container-lowest)' }}>
            ...
            <ol style={{ margin: 0, paddingLeft: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 9 }}>
              {[['key', 'Authentik verifies your identity (PKCE)'], ['groups', 'Your groups claim derives your role'], ['login', 'You land on your role-aware dashboard']].map(([ic, t], i) => (
                <li key={i} style={{ display: 'flex', alignItems: 'center', gap: 9, fontSize: 12, color: 'var(--on-surface-variant)' }}>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, fontWeight: 700, width: 16, height: 16, borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'color-mix(in srgb, var(--primary) 14%, transparent)', color: 'var(--primary)' }}>{i + 1}</span>
                  {t}
                </li>
              ))}
            </ol>
